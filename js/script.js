    ...
    const calendar = `<div class="container">
                    ...
                    <div class="card border-0 mt-5 flex-fill d-none" id="event">
                        ...
                        <div class="card-body px-1 py-3">
                            <div class="text-center">
                                <span class="event-date">06 Junio 2020</span><br>
                                <span class="event-day">Lunes</span>
                            </div> 
                            <div class="events-today my-3 px-3">
                               
                            </div> 
                            <div class="input-group events-input mb-3 col-10 mx-auto mt-2">
                                <input type="text" class="form-control" placeholder="Su evento" id="eventTxt">
                                <div class="input-group-append">
                                    <button class="btn btn-info" type="button" id="createEvent">+</button>
                                </div>
                            </div>                        
                        </div>
                    </div>                            
        ...
        <div aria-live="polite" aria-atomic="true" style="position: relative; min-height: 200px;">
            <div class="toast" style="position: absolute; top: 0; right: 15px;" data-delay="3000">
                ...
                <strong class="mr-auto">Calendario</strong>
    ...
    $(function(){
        function showEvent(eventDate){
            ...
            if (storedEvents == null){
                $('.events-today').html('<h5 class="text-center">No se han encontrado eventos</h5 class="text-center">');               
            }else{
                ...
                if(eventsList.length>0){
                    let eventsLi ='';
                    eventsList.forEach(event =>  $('.events-today').html(eventsLi +=`<div class="alert alert-info alert-dismissible fade show" role="alert">
                    ${event.eventText}
        ...
        function removeEvent(id){
            ...
            if(storedEvents != null){
                ...
                $('.toast-body').html('Su evento se ha eliminado');
                $('.toast').toast('show');
            }
        }        
        ...
        $(document).on('click', '#createEvent', function(){
            ...
            $('.error').remove();
            if (eventText == ''){
                $('.events-input').append(`<span class="error">Debe de añadir un evento</span>`);
                $('#eventTxt').addClass('data-invalid');
                $('#eventTxt').trigger('focus');
            }else if(eventText.length < 3){
                $('#eventTxt').addClass('data-invalid');
                $('#eventTxt').trigger('focus');
                $('.events-input').append(`<span class="error">Tiene que añadir mínimo 3 carácteres</span>`);
            }else{
                valid = true;
            }
            if (valid){
                ...
                $('#eventTxt').val('');
                $('.toast-body').html('Su evento se ha añadido');
                $('.toast').toast('show');
                showEvent(eventDate);
            }
        })
